...
class design_area extends connect{
    ...
    private $queryGetAll = 'SELECT areas.name_area, staff.first_name, position.name_position, journey.name_journey  FROM design_area INNER JOIN areas ON design_area.id_area = areas.id INNER JOIN staff ON design_area.id_staff = staff.id INNER JOIN position ON design_area.id_position = position.id INNER JOIN journey ON design_area.id_journey = journey.id';

    private $queryGet = 'SELECT areas.name_area, staff.first_name, position.name_position, journey.name_journey  FROM design_area INNER JOIN areas ON design_area.id_area = areas.id INNER JOIN staff ON design_area.id_staff = staff.id INNER JOIN position ON design_area.id_position = position.id INNER JOIN journey ON design_area.id_journey = journey.id WHERE id = ?';
